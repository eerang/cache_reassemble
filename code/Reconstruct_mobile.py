...
def get_section_from_filename(filename):
    section_filename = filename.split(".-")[0]

    return section_filename

def get_fragment_from_filename(filename):
    fragment_filename = filename.split(".mp4")[1].split(".")[1]
    #print(fragment_filename)
...
def parse_filename(filename_lists):
    categorized_by_app_and_section = dict()

    #for filename_list in filename_lists :
    for (file_name, file_name_2) in filename_lists:
        app_information = "A"
        section_information = get_section_from_filename(file_name_2)

        if app_information not in categorized_by_app_and_section.keys() :
            categorized_by_app_and_section.setdefault(app_information, dict())
        if section_information not in categorized_by_app_and_section[app_information].keys():
            categorized_by_app_and_section[app_information].setdefault(section_information, list())

        categorized_by_section = categorized_by_app_and_section[app_information][section_information]
        categorized_by_section.append((file_name,get_fragment_from_filename(file_name_2)))

    return categorized_by_app_and_section


if __name__ == '__main__':

    #basepath = "D://0.DFRC//2.논문//연구//에코쇼//Analysis//Echo//Cache//youtube_org.chromium.android_webview"
    #basepath = "D://EXO//ttestp_webm"
    basepath = "C://Users//yirang//Desktop//ExoPlayerCacheDir//videocache"
    data = []
    result = []
    ...
    for file_name in os.listdir(basepath):
        target = os.path.join(basepath, file_name)
        data.append(file_name)
        #file_list = parse_filename(file_name)
        data.append(file_name)
    ...
    file_name_and_file_name = [data[i * n:(i + 1) * n] for i in range((len(data) + n - 1) // n)]

    result_of_parse = parse_filename(file_name_and_file_name)  # url parsing
    result_of_sort = sorting(result_of_parse)  # sorting
